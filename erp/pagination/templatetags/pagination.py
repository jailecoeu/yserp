...
class GPaginator(Paginator):
    """ 直接传入数据数，避免Queryset的count()，Postgresql的数据统计 """
    
    def __init__(self, object_list, per_page, orphans=0, rows=None, allow_empty_first_page=True):
...
def do_autopaginate(parser, token):
    ...
    {% autopaginate queryset 5 0 rows %}
    """
    ...
    if len(split) == 2:
        ...
        if len(split) == 4:
            ...
            return AutoPaginateNode(split[1], paginate_by=split[2], orphans=orphans, rows=split[4], context_var=context_var)
    else:
        raise template.TemplateSyntaxError('%r tag takes one required rgument and one optional argument' % split[0])
...
class AutoPaginateNode(template.Node):
    ...
    """
    def __init__(self, queryset_var, paginate_by=DEFAULT_PAGINATION, orphans=DEFAULT_ORPHANS, rows=None, context_var=None):
    ...
    def render(self, context):
        ...
        if self.rows is not None:
            try:
                self.rows = int(template.resolve_variable(self.rows, context))
                assert self.rows >= 0
            except:
                self.rows = None
        
        paginator = GPaginator(value, paginate_by, self.orphans, self.rows)
        try:
